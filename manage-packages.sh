...
list_packages() {
    ...
    print_info "Listing $package_type packages for organization: $ORG"
    
    gh api "/orgs/$ORG/packages?package_type=$package_type" | \
        jq -r '.[] | "\(.id)\t\(.name)\t\(.updated_at)"' | \
        column -t -s $'\t' -N "ID,Name,Updated"
}

# List packages with details
list_packages_detailed() {
    local package_type=${1:-$PACKAGE_TYPE}
    print_info "Listing detailed $package_type packages for organization: $ORG"
    
    gh api "/orgs/$ORG/packages?package_type=$package_type" | \
        jq -r '.[] | "\(.id)\t\(.name)\t\(.visibility)\t\(.updated_at)\t\(.repository.name // "N/A")"' | \
        column -t -s $'\t' -N "ID,Name,Visibility,Updated,Repository"
}
...
delete_packages_pattern() {
    ...
    packages=$(gh api "/orgs/$ORG/packages?package_type=$package_type" | \
        jq -r ".[] | select(.name | contains(\"$pattern\")) | .name")
...
delete_all_packages() {
    ...
    print_error "WARNING: This will delete ALL $package_type packages in organization $ORG"
...
list_versions() {
    ...
    gh api "/orgs/$ORG/packages/$package_type/$package_name/versions" | \
        ...
        column -t -s $'\t' -N "ID,Version,Created"
}
